...
apiRouter.post("/addElectronicSignatoryWithDocIdentity", async (req: Request, res: Response) => {
	...
	try {
		...
		if (signature_params.doc_identidad) {
			const normalized_imagen_front_document = signature_params.imagen_front_document.startsWith("/") ? signature_params.imagen_front_document.slice(1) : signature_params.imagen_front_document
			const normalized_imagen_behind_document = signature_params.imagen_behind_document.startsWith("/") ? signature_params.imagen_behind_document.slice(1) : signature_params.imagen_behind_document

			signature_params.path_imagen_front_document = await s3Repository.getTempPathFromURI_JPG(`public/${normalized_imagen_front_document}`)
			signature_params.path_imagen_behind_document = await s3Repository.getTempPathFromURI_JPG(`public/${normalized_imagen_behind_document}`)
		}
		...
		if (signature_params.doc_identidad) {
			...
			filerepository.deleteFile(signature_params.path_imagen_behind_document)
		}
